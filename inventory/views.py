...
from .forms import MaterialForm, LocationForm, MovimientoInventarioForm, UserRegisterForm, SearchForm, LocationSelectForm
...
def almacenero_view(request):
    ...
    if request.method == "POST":
        ...
        if 'add_material' in request.POST:
            ...
            material_form = MaterialForm(request.POST, request.FILES, instance=material_instance)
            if material_form.is_valid():
                material_form.save()
...
@login_required
@permission_required('inventory.can_register_movement', raise_exception=True)
def registrar_movimiento(request):
    return render(request, 'inventory/registrar_movimiento.html')
...
def compra_view(request):
    ...
    return render(request, 'inventory/compra.html', {'materials': materials, 'cart': cart})

@login_required
def add_to_cart(request, material_id):
    material = get_object_or_404(Material, id=material_id)
    cart_item, created = CartItem.objects.get_or_create(user=request.user, material=material)
    if not created:
        cart_item.quantity += 1
        cart_item.save()
    messages.success(request, 'Material agregado al carrito.')
    return redirect('compra')

@login_required
def remove_from_cart(request, material_id):
    cart_item = get_object_or_404(CartItem, user=request.user, material_id=material_id)
    cart_item.delete()
...
def finalize_purchase(request):
    ...
    if cart_items:
        # Procesar la compra aquí (e.g., crear una orden, reducir el stock, etc.)
        cart_items.delete()
